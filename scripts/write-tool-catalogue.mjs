...
w("  faster. Past the size one read can hold, a run from a water source you");
w("  placed still asks for a smaller study area -- that one is a traversal and");
w("  has to be bounded -- but a plain \"everything below this level\" run is");
w("  counted band by band over the whole survey instead, at full resolution,");
w("  and drawn as a tile layer rather than as polygons. Where a spill surface");
w("  has been built, the same run can instead trace only water arriving from");
w("  outside the survey, so a hollow with no path to it stays dry. Every level");
w("  of a bounded run exports as a polygon carrying its water level, interval");
w("  and area, as GeoJSON or as a real shapefile. See `docs/tools.md`.");
w();
