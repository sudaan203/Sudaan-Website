...
export const getSession = cache(async (): Promise<PortalSession | null> => {
  ...
  // Google is the only way in, so anything else is a cookie issued before that
  // was true. Those are refused rather than migrated: a password session was
  // minted against a list that no longer exists, so there is nothing left to
  // re-check it against, and the holder can sign in again with Google in one
  // click. This is the line that ends any session still live at deploy time.
  if (session.via !== "google") return null;

  // Sessions are stateless JWTs, so revoking one means checking the source of
  // truth on every request: the users table, where an owner deactivates someone.
